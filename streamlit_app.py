...
def poisson_probability(actual_goals, mean_goals):
    return (np.exp(-mean_goals) * (mean_goals ** actual_goals)) / np.math.factorial(actual_goals)

# Team statistics
team_A_avg_goals = 1.3
team_B_avg_goals = 1.7

# Calculate probabilities for each scoreline (0-5 goals)
max_goals = 5
score_matrix = []
for goals_A in range(max_goals + 1):
    row = []
    for goals_B in range(max_goals + 1):
        prob_A = poisson_probability(goals_A, team_A_avg_goals)
        prob_B = poisson_probability(goals_B, team_B_avg_goals)
        row.append(prob_A * prob_B)
    score_matrix.append(row)

# Convert to DataFrame for analysis
score_df = pd.DataFrame(score_matrix, columns=[f"B:{i}" for i in range(max_goals + 1)], index=[f"A:{i}" for i in range(max_goals + 1)])

# Flatten the matrix for sorting
flattened_scores = []
for i, row in enumerate(score_matrix):
    for j, prob in enumerate(row):
        flattened_scores.append((f"{i}:{j}", prob))

# Sort scorelines by probability
sorted_scores = sorted(flattened_scores, key=lambda x: x[1], reverse=True)

# Approximate the top 12 scorelines for analysis
top_12 = sorted_scores[:12]

# Add a matrix-based recommendation logic (5.28% to 6.19% range)
matrix_recommendation = [score for score, prob in top_12 if 0.0528 <= prob <= 0.0619]

# Final recommendation
if matrix_recommendation:
    final_recommendation = matrix_recommendation[0]
    final_probability = [prob for score, prob in top_12 if score == final_recommendation][0]
else:
    final_recommendation = top_12[0][0]
    final_probability = top_12[0][1]

# Display results
print("Top 12 Most Likely Scorelines:")
for score, prob in top_12:
    print(f"Scoreline: {score}, Probability: {prob * 100:.2f}%")
